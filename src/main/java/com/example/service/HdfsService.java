...
public class HdfsService {
    ...
    public void createLocalFile() throws Exception {
        ...
        FileInputStream fis = new FileInputStream(new File("/Users/baidu/Desktop/111.txt"));

        FileSystem fs = getFileSystem();
        // 上传时默认当前目录，后面自动拼接文件的目录
        Path newPath = new Path(  "/user/yuan/input/a");
        // 打开一个输出流
        FSDataOutputStream outputStream = fs.create(newPath);

        IOUtils.copyBytes(fis,outputStream,getConfiguration());
